...
import LwwTwoPhaseSet, { SetElement } from '../lwwTwoPhaseSet.js';
...
describe('LwwTwoPhaseSet', () => {
  ...
  describe('add', () => {
    test('should add a SetElement to addSet', () => {
      ...
      expect(newElement.value).toBe('a');
      expect(newElement.timestamp).toBe(1);
    });
    ...
    test('should add a SetElement with Date.now() to addSet', () => {
      ...
      expect(newElement instanceof SetElement).toBe(true);
      expect(newElement.value).toBe('a');
      expect(newElement.timestamp).toBe(1560312000);
    });
  });
  ...
  describe('remove', () => {
    test('should add a SetElement to removeSet when addSet contains the element', () => {
      ...
      expect(newElement instanceof SetElement).toBe(true);
      expect(newElement.value).toBe('a');
      expect(newElement.timestamp).toBe(2);
    });
    ...
    test('should throw an error when addSet contains the element', () => {
      expect(() => twoPhaseSet.remove('a', 2)).toThrow(
        'a element does not exist or has been removed'
      );
  ...
  describe('lookupAll', () => {
    test('should return an array with existing elements', () => {
      ...
      expect(`${[...twoPhaseSet.lookupAll()]}`).toBe(`${['a', 'b']}`);
    });
  });
});
